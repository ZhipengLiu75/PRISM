...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_batch_size', type=int, default=16, help='test_batch_size')
    ...
    parser.add_argument('--grad_clip', type=int, default=1, help='whether to clip gradient norm')
    parser.add_argument('--max_norm', type=float, default=1.0, help='maximum gradient norm')
    ...
    parser.add_argument('--model_id', type=str, required=False,
                        default='ETTm1_PRISM_MSD_96_96_B24', help='model id')
    parser.add_argument('--model', type=str, required=False, default='PRISM',
    ...
    # OLinear_CycleRefine / Adaptive-L parameters
    parser.add_argument('--cycle_len', type=int, default=24, help='cycle length used by cycle refinement')
    parser.add_argument('--cycle_refine_ratio', type=float, default=0.03, help='initial cycle refinement ratio')
    parser.add_argument('--cycle_hidden_mult', type=int, default=2, help='hidden multiplier for cycle refinement')
    parser.add_argument('--use_horizon_calibration', type=int, default=0,
                        help='enable horizon-conditioned multi-anchor calibration')
    parser.add_argument('--horizon_calibration_cycle', type=int, default=24,
                        help='period used by multi-anchor calibration')
    parser.add_argument('--horizon_calibration_hidden', type=int, default=32,
                        help='content-gate hidden dimension')
    parser.add_argument('--horizon_calibration_rank', type=int, default=8,
                        help='low-rank horizon gate dimension')
    parser.add_argument('--horizon_calibration_ratio', type=float, default=0.01,
                        help='initial residual calibration strength')
    parser.add_argument('--use_adaptive_l', type=int, default=0, help='whether to use adaptive input length')
    parser.add_argument('--min_context_len', type=int, default=24, help='minimum adaptive context length')
    parser.add_argument('--lambda_context', type=float, default=0.0, help='context length regularization weight')
    parser.add_argument('--crop_init_ratio', type=float, default=0.02, help='initial adaptive crop ratio')
    parser.add_argument('--boundary_hidden', type=int, default=64, help='hidden size of boundary predictor')
    parser.add_argument('--context_tau', type=float, default=12.0, help='temperature of adaptive length mask')
    parser.add_argument('--normalize_adaptive_mask', type=int, default=0, help='normalize adaptive mask')
    parser.add_argument('--mask_norm_eps', type=float, default=1e-4, help='adaptive mask norm epsilon')
    parser.add_argument('--mask_norm_clip', type=float, default=3.0, help='adaptive mask norm clip')

    # FORT: causal feedback-observed residual transport
    parser.add_argument('--fort_block_size', type=int, default=96)
    parser.add_argument('--fort_rank', type=int, default=16)
    parser.add_argument('--fort_ridge', type=float, default=1.0)
    parser.add_argument('--fort_basis_hidden', type=int, default=64)
    parser.add_argument('--fort_gate_hidden', type=int, default=32)
    parser.add_argument('--fort_gate_init', type=float, default=-1.0)
    parser.add_argument('--fort_lambda_prog', type=float, default=1.0)
    parser.add_argument('--fort_lambda_mono', type=float, default=0.1)
    parser.add_argument('--fort_lambda_smooth', type=float, default=1e-3)
    parser.add_argument('--fort_mono_margin', type=float, default=0.0)
    parser.add_argument('--fort_detach_feedback', type=int, default=0)
    parser.add_argument('--fort_backbone_checkpoint', type=str, default='')
    parser.add_argument('--fort_freeze_backbone', type=int, default=0)

    # SHIFT: hindsight-guided causal hypothesis filtering
    parser.add_argument('--shift_block_size', type=int, default=96)
    parser.add_argument('--shift_scenarios', type=int, default=8)
    parser.add_argument('--shift_rank', type=int, default=8)
    parser.add_argument('--shift_hidden', type=int, default=64)
    parser.add_argument('--shift_embedding_dim', type=int, default=16)
    parser.add_argument('--shift_teacher_temperature', type=float, default=0.1)
    parser.add_argument('--shift_posterior_temperature', type=float, default=1.0)
    parser.add_argument('--shift_posterior_gamma', type=float, default=1.0)
    parser.add_argument('--shift_scenario_scale', type=float, default=0.1)
    parser.add_argument('--shift_lambda_coverage', type=float, default=0.2)
    parser.add_argument('--shift_lambda_bridge', type=float, default=1.0)
    parser.add_argument('--shift_lambda_contract', type=float, default=0.1)
    parser.add_argument('--shift_lambda_balance', type=float, default=0.01)
    parser.add_argument('--shift_lambda_diversity', type=float, default=0.01)
    parser.add_argument('--shift_lambda_progressive', type=float, default=1.0)
    parser.add_argument('--shift_lambda_monotonic', type=float, default=0.1)
    parser.add_argument('--shift_backbone_cycle', type=int, default=24)
    parser.add_argument('--shift_backbone_factor_rank', type=int, default=8)
    parser.add_argument('--shift_backbone_hidden', type=int, default=64)
    parser.add_argument('--shift_backbone_dropout', type=float, default=0.1)
    parser.add_argument('--trace_block_size', type=int, default=96)
    parser.add_argument('--trace_basis_dim', type=int, default=6)
    parser.add_argument('--trace_margin', type=int, default=24)
    parser.add_argument('--trace_ridge', type=float, default=1.0)
    parser.add_argument('--trace_max_shift', type=float, default=24.0)
    parser.add_argument('--trace_tangent_floor', type=float, default=1e-3)
    parser.add_argument('--trace_lambda_prog', type=float, default=1.0)
    parser.add_argument('--trace_lambda_stab', type=float, default=0.01)
    parser.add_argument('--trace_clock_hidden', type=int, default=32)
    parser.add_argument('--trace_alignment_temperature', type=float, default=0.05)
    parser.add_argument('--trace_dynamics_hidden', type=int, default=64)
    # PRISM: progressive input re-encoding and progressive output rollout
    parser.add_argument('--prism_block_size', type=int, default=24)
    parser.add_argument('--prism_hidden', type=int, default=64)
    parser.add_argument('--prism_lambda_progressive', type=float, default=1.0)
    parser.add_argument('--prism_modulation_scale', type=float, default=0.1)
    # ReCast: editable forecasts with response-path consistency
    parser.add_argument('--recast_block_size', type=int, default=96)
    parser.add_argument('--recast_rank', type=int, default=16)
    parser.add_argument('--recast_hidden', type=int, default=96)
    parser.add_argument('--recast_channel_dim', type=int, default=8)
    parser.add_argument('--recast_local_modes', type=int, default=4)
    parser.add_argument('--recast_basis_hidden', type=int, default=64)
    parser.add_argument('--recast_lambda_revision', type=float, default=1.0)
    parser.add_argument('--recast_lambda_path', type=float, default=0.1)
    parser.add_argument('--recast_max_edit', type=float, default=4.0)
    ...
    # Keep compatibility with older code using args.cycle.
    # The new OLinear_CycleRefine model uses args.cycle_len.
    if not hasattr(args, 'cycle'):
        args.cycle = args.cycle_len
    ...
    if args.is_training and not args.model_stats_mode and not args.save_linear_weight:
        ...
        for ii in range(args.itr):
            ...
            if args.test_mode == 0:
                ...
                print(
                    ...
                    f'(batch_size:{args.batch_size}, lamda1:{args.lamda1:.2f}, '
                    f'alpha:{args.alpha:.2f}, lossfun_alpha:{args.lossfun_alpha:.2f}) '
                    f'(iteration:{ii + 1}/{args.itr}) '
            ...
            result_string = (
                f'mse:{mse:.5f}, mae:{mae:.5f}, lamda1:{args.lamda1:.2f}, '
            ...
            print(
                f'training time left is '
                f'{np.mean(time_vec) * (args.itr - ii - 1) / 60.0:.2f} min...'
            )

            iteration_summary = (
                f'best_mse (by far): {best_mse:.5f}, best_mae: {best_mae:.5f};\t '
        ...
        if time_vec:
            global_summary = (
                f'global best_mse: {best_mse:.5f}, best_mae: {best_mae:.5f};\t '
                f'git_multi_stage:{args.git_multi_stage}, '
                f'alpha1:{args.alpha}, loss_fun_alpha1:{args.lossfun_alpha}, '
